...
SELECT B.GUESTNO, B.BOOKINGNO, S.DISCOUNT as DISCOUNT, R.PRICE * EXTRACT(DAY FROM NUMTODSINTERVAL(DATETO - DATEFROM, 'DAY')) AS ROOM_PRICE, SUM(A.PRICE * AU.QUANTITY) AS AMENITIES, ((100 - S.DISCOUNT)/100 * (R.PRICE * EXTRACT(DAY FROM NUMTODSINTERVAL(DATETO - DATEFROM, 'DAY')) + SUM(A.PRICE * AU.QUANTITY))) AS TOTAL_BILL
FROM TAOSEEFAZIZ.ATAK_BOOKING B
JOIN TAOSEEFAZIZ.ATAK_GUEST G ON G.GUESTNO = B.BOOKINGNO
JOIN TAOSEEFAZIZ.ATAK_SUBSCRIPTION S ON S.SUBNO = G.SUBNO
JOIN TAOSEEFAZIZ.ATAK_ROOM R ON B.ROOMNO = R.ROOMNO
